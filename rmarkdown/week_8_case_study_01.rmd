...
Use KNN classification to investigate this relationship.  Provide statistical evidence one way or the other. 
...
merged_beer <- omitBeerBrewDF
ggplot(data = merged_beer) +
  geom_col(mapping = aes(x=ABV, y=IBU, fill = Ounces)) + 
  facet_wrap(~Ounces)
...
ggplot(data = merged_beer, aes(x=ABV*100, y=IBU)) + geom_point() + 
  geom_smooth(method = lm) +
  facet_wrap(~Ounces) + ggtitle("ABV vs IBU: Trends per Serving Size") +
  xlab("ABV (%)")

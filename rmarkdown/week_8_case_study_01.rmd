...
#
#BSDF <- breweries %>% group_by(State) %>% distinct(Brew_ID) 
#
#BrewCount <- aggregate(Brew_ID ~ State, breweries, length)
#
#names(BrewCount) <- c("State", "Breweries")
#
#BrewCount$State <- trim(BrewCount$State, recode.factor = TRUE) 
#
#BrewCount$State <- tolower(abbr2state(BrewCount$State))
#
#BrewMap <- left_join(US_States, BrewCount, by = "State")
#
#Coords <- aggregate(cbind(long, lat) ~ State, data = BrewMap, FUN = function(x) mean(range(x))) 
#
##bug here with vars() anonymous namespace, looks like it is losing track of lat and long objects
#Coords2 <- BrewMap %>% group_by(State) %>% summarize_at(vars(long, lat), ~ mean(range(.))) 
#
